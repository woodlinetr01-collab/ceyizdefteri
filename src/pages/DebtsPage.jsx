...
export default function DebtsPage() {
  ...
  const [person, setPerson] = useState(""); const [direction, setDirection] = useState("alacak");
  const [amount, setAmount] = useState(""); const [dueDate, setDueDate] = useState(todayIso()); const [note, setNote] = useState("");
  ...
  const submit = () => {
    ...
    dispatch({ type: "ADD_DEBT", payload: { person, direction, amount: Number(amount), dueDate, note } });
    setOpen(false); setPerson(""); setAmount(""); setNote("");
  };
    ...
    <div className="page">
      ...
      <div className="two-col">
        <div className="panel">
          ...
          {live.filter((d) => d.direction === "alacak").length === 0 ? <EmptyState text="Kayıt yok." /> : live.filter((d) => d.direction === "alacak").map((d) => <DebtRow d={d} key={d.id} />)}
        </div>
        <div className="panel">
          <h3 className="panel-h">Benim Ödeyeceğim</h3>
          {live.filter((d) => d.direction === "borc").length === 0 ? <EmptyState text="Kayıt yok." /> : live.filter((d) => d.direction === "borc").map((d) => <DebtRow d={d} key={d.id} />)}
        </div>
      </div>
      ...
      {open && (
        <div className="modal-backdrop" onClick={() => setOpen(false)}>
          <div className="modal modal-sm" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head"><div><Eyebrow>Yeni Kayıt</Eyebrow><h3>Borç / Alacak Ekle</h3></div><button className="icon-btn" onClick={() => setOpen(false)}><X size={18} /></button></div>
