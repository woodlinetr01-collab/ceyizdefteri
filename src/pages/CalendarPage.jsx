...
export default function CalendarPage() {
  ...
  const events = useMemo(() => {
    ...
    calc.loansComputed.forEach((loan) => {
      loan.schedule.filter((l) => l.status !== "odendi" && l.status !== "iptal").forEach((l) => {
        list.push({ date: l.dueDate, desc: `${loan.name} · ${l.no}/${l.of}. Taksit`, amount: l.remaining, category: null, status: l.status, kind: "kredi" });
      });
    });
    return list;
  }, [calc]);
