            ...
            return (
              <div className="budget-row" key={r.key}>
                <div className="budget-row-top">
                  ...
                  <span className="mono">{fmtTL(r.actual)} <span className="muted">/ <EditablePlanned categoryKey={r.key} planned={r.planned} /></span></span>
                </div>
...
/** Zaten planlanmış bir kategori bütçesini yerinde (inline) düzenlemeyi sağlar. */
function EditablePlanned({ categoryKey, planned }) {
  const { dispatch } = useAppData();
  ...
  if (!editing) {
    return (
      <button type="button" className="editable-planned" onClick={() => { setValue(String(planned)); setEditing(true); }} title="Planlanan bütçeyi düzenle">
        {fmtTL(planned)}
      </button>
    );
  }
  const save = () => {
    ...
    if (!isNaN(n) && n >= 0) dispatch({ type: "SET_BUDGET", category: categoryKey, amount: n });
    setEditing(false);
  };
  return (
    <span className="inline-pay">
      <input type="number" autoFocus min="0" value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => e.key === "Enter" && save()} style={{ width: 90 }} />
      <button className="pill-btn on" onClick={save}>Kaydet</button>
    </span>
  );
}
