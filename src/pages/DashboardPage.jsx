...
export default function DashboardPage() {
  ...
  const cardLimitTotal = calc.cardsComputed.reduce((s, c) => s + c.limit, 0);
  const cardUsedTotal = calc.cardsComputed.reduce((s, c) => s + c.usedLimit, 0);
  ...
  return (
    <div className="page">
      ...
      <div className="sum-grid">
        <SummaryCard label="Devreden Bakiye" value={fmtTL(calc.carryOverBalance)} sub="Önceki aydan" icon={PiggyBank} tone={calc.carryOverBalance < 0 ? "rose" : "sky"} />
        ...
        <SummaryCard
          ...
          value={cardLimitTotal > 0 ? `${fmtTL(cardUsedTotal)} / ${fmtTL(cardLimitTotal)}` : "Kart yok"}
          sub={cardLimitTotal > 0 ? `%${Math.round((cardUsedTotal / cardLimitTotal) * 100)} kullanım` : null}
          icon={CardIcon} tone="sky"
        />
      ...
      <div className="panel">
        <SectionTitle kicker="Tahmin" title="Gelecek Ay Tahmini" />
        <div className="sum-grid sum-grid-3">
          <SummaryCard label="Tahmini Gelir" value={fmtTL(calc.nextMonthForecast.income)} tone="ink" />
          <SummaryCard label="Kart Taksitleri" value={fmtTL(calc.nextMonthForecast.cardInstallments)} tone="gold" />
          <SummaryCard label="Kredi Taksitleri" value={fmtTL(calc.nextMonthForecast.loanPayments)} tone="gold" />
          <SummaryCard label="Diğer Sabit Giderler" value={fmtTL(calc.nextMonthForecast.otherExpense)} tone="ink" />
          <SummaryCard label="Devreden Bakiye" value={fmtTL(calc.nextMonthForecast.carryIn)} tone="sky" />
          <SummaryCard label="Tahmini Serbest Bakiye" value={fmtTL(calc.nextMonthForecast.expectedFree)} tone={calc.nextMonthForecast.expectedFree < 0 ? "rose" : "emerald"} />
        </div>
      </div>
